...
df_intervention_day_3 <- REDCapR::redcap_report(redcap_uri = "https://redcap.ucsf.edu/api/", token = Sys.getenv("token_protocol"), report_id = 241587L, guess_type = F)$data

df_intervention_day_4 <- REDCapR::redcap_report(redcap_uri = "https://redcap.ucsf.edu/api/", token = Sys.getenv("token_protocol"), report_id = 241588L, guess_type = F)$data

df_intervention_day_5 <- REDCapR::redcap_report(redcap_uri = "https://redcap.ucsf.edu/api/", token = Sys.getenv("token_protocol"), report_id = 241589L, guess_type = F)$data

df_intervention_day_6 <- REDCapR::redcap_report(redcap_uri = "https://redcap.ucsf.edu/api/", token = Sys.getenv("token_protocol"), report_id = 241590L, guess_type = F)$data

df_intervention_day_7 <- REDCapR::redcap_report(redcap_uri = "https://redcap.ucsf.edu/api/", token = Sys.getenv("token_protocol"), report_id = 241591L, guess_type = F)$data



#combine all dias of intervention, filtered where the drug day of intervention question for that day is ==1 
all_days <- df_intervention_day_1 %>% 
  filter(redcap_repeat_instrument=="intervencion_day_1") %>% 
  filter(drug_day_of_intervention_1=="1") %>% # (149)
  distinct(preint_partcode, .keep_all = T)%>%
  left_join(
  ...
  left_join(
    df_intervention_day_3 %>% 
      filter(drug_day_of_intervention_3=="1")%>%
      distinct(preint_partcode_3, .keep_all = T),
    by = c("preint_partcode"="preint_partcode_3")) %>%
  
  left_join(
    df_intervention_day_4 %>% 
      filter(drug_day_of_intervention_4=="1")%>%
      distinct(preint_partcode_4, .keep_all = T),
    by = c("preint_partcode"="preint_partcode_4")) %>%
  left_join(
    df_intervention_day_5 %>% 
      filter(drug_day_of_intervention_5=="1")%>%
      distinct(preint_partcode_5, .keep_all = T),
    by = c("preint_partcode"="preint_partcode_5")) %>%
  left_join(
    df_intervention_day_6 %>% 
      filter(drug_day_of_intervention_6=="1")%>%
      distinct(preint_partcode_6, .keep_all = T),
    by = c("preint_partcode"="preint_partcode_6")) %>%
  left_join(
    df_intervention_day_7 %>% 
      filter(drug_day_of_intervention_7=="1")%>%
      distinct(preint_partcode_7, .keep_all = T),
    by = c("preint_partcode"="preint_partcode_7"))
...
length(unique(all_days$preint_partcode))
