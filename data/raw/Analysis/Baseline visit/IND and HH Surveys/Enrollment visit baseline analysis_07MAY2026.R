...
#create a new g6pd categorical variable
  #basal$base_ind_g6pd_result <- as.numeric(as.character(basal$base_ind_g6pd_result))
basal$base_ind_g6pd_result <- as.numeric(basal$base_ind_g6pd_result)
...
basal$base_ind_g6pd_hemo_result <- as.numeric(basal$base_ind_g6pd_hemo_result)
...
ggplot(basal, aes(x = base_ind_g6pd_result, fill = base_ind_sex, color=base_ind_sex)) +
  geom_density(alpha = 0.5, position = "identity")+
  scale_fill_manual(values= c("blue", "red"))+
  scale_color_manual(values = c("blue", "red")) +
  scale_x_continuous(breaks = seq(0, max(basal$base_ind_g6pd_result, na.rm = TRUE), by = 1)) + # Tick marks every 1 unit
  labs(x = "G6PD enzyme activity IU/g Hb",y = "Frequency", fill = "Sex", color = "Sex") +
  theme_minimal() +
  theme(
...
unique_communities <- unique(basal$community_code)

for (community in unique_communities) {
  cat("Community:", community, "\n")
  
 avg_g6pd <- basal %>%
   filter(community_code == community) %>%
   summarize(mean_g6pd = mean(base_ind_g6pd_result, na.rm = TRUE))
 
 
 result_list[[community]] <- avg_g6pd$mean_g6pd
 
 cat("Community:", community, "Mean G6PD:", avg_g6pd$mean_g6pd, "\n")
 
}

result_df <- data.frame(
  community_code = names(result_list),
  mean_g6pd = unlist(result_list)
)




ggplot(basal, aes(x = community_code, y = base_ind_g6pd_result, fill=base_ind_sex)) + 
  geom_boxplot()+
  labs(x= "Community", y = "G6PD IU/g Hb", fill="Sex")+
  theme_light()


ggplot(basal, aes(x = community_code, y = base_ind_g6pd_result)) + 
  geom_boxplot()+
  labs(x= "Community", y = "G6PD IU/g Hb")+
  theme_light()
...
table(basal$g6pd_category, useNA = 'always')
...
basal %>%
  filter(base_ind_g6pd_class %in% c("G6PD intermedio")) %>%
  select(base_ind_code,base_ind_g6pd_class, base_ind_sex, base_ind_age, base_ind_g6pd_result) %>%
  arrange(factor(base_ind_sex)) 
...
ggplot(basal, aes(x = community_code, y = base_ind_g6pd_hemo_result, fill=base_ind_sex)) + 
  geom_boxplot()+
  labs(x= "Community", y = "Hemoglobin", fill="Sex")+
  theme_light()
...
ggplot(basal, aes(x = age_category, y = base_ind_g6pd_hemo_result, fill = base_ind_sex)) + 
  ...
  ylim(5,20)+
  scale_fill_manual(values = c("Male" = "#79c36a", "Female" = "skyblue"))+
  theme_light()+
  theme(axis.title = element_text(size=14,face="bold"),
...
##G6PD AND HB LEVELS TOGETHER BY SEX##
...
#Plot both Hb and G6PD together on same plot, G6PD scaled to Hb
ggplot() +
...
house <- read.csv('FLAME-Hhdatareportsf_10JUN2025.csv')

#elimnate rows where the hh code is NA
house <- house %>% filter(!is.na(base_hh_house_code))

#Creating labels for wall material
house$base_hh_mat_wall <- factor(house$base_hh_mat_wall, levels = c(1,2,3,4,5,6,7,99), labels = c(
  "Ladrillo, cemento", "Madera","Adobe", "Paja", "Palmera (hoja)", "Triplay", "Esteras", "Otro"))
...
house$base_hh_wtr_source <- factor(house$base_hh_wtr_source, levels = c(1,2,3,4,5,6,99), labels = c(
  "Intubada fuera/dentro de casa", "Pozo", "Pilón uso público", "Lluvia", "Pozo artesiano", "Río o quebrada", "Otro"))
...
####ENROLLMENT OVERALL BY COMMUNITY####

#Read in dataset with data from baseline survey- individual level
enroll<- read_excel('HH and INDIV enrolled_18DEC2024.xlsx')

#Set Community variable as a factor variable
enroll$Community <- factor(enroll$Community, levels = unique(enroll$Community))


#Plot % of HH enrolled per community
ggplot(enroll, aes(x = Community, y=`PCT HH Enrolled`)) + 
  geom_bar(stat = "identity", color="black", fill="#599ad3")+
  geom_text(aes(label = round(`PCT HH Enrolled`, 1)), 
            vjust = -0.5, size = 3.0) +
  labs(x="", y ="Household Enrollment %")+
  ylim(0,100)+
  theme_minimal()+
  theme(axis.title = element_text(size=14,face="bold"),
        axis.text.y = element_text(size = 13), 
...
#Plot % of IND enrolled per community
ggplot(enroll, aes(x = Community, y=`PCT IND Enrolled`)) + 
  geom_bar(stat = "identity", color="black", fill="#f9a65a")+
  geom_text(aes(label = round(`PCT IND Enrolled`, 1)), 
            vjust = -0.5, size = 3.0) +
  labs(x="", y = "Individual Enrollment %")+
  ylim(0,100)+
  theme_minimal()+
  theme(axis.title = element_text(size=14,face="bold"),
        axis.text.y = element_text(size = 13), 
